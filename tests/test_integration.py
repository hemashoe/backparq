...
from backparq.config import BackparqConfig, DatabaseConfig, S3Config, ParquetConfig, ArchiveConfig, CronConfig
from backparq.archive import archive_tables
from backparq.restore import restore_tables
from backparq.db import connect_pg
...
def test_archive_and_restore(postgres, db_config, s3_config, tmp_path):
    ...
    config = BackparqConfig(
        ...
        parquet=ParquetConfig(
             encryption=ParquetEncryptionConfig(enabled=False)
        ),
        archive=ArchiveConfig(
            tables=["test_events"],
            cutoff_exclusive=dt.datetime(2023, 2, 1, tzinfo=dt.timezone.utc), # Archive Jan, keep Feb
            base_dir=tmp_path,
            perform_delete=True,
    ...
    with engine.connect() as conn:
        ...
        jan_count = conn.execute(text("SELECT count(*) FROM test_events WHERE created_at < '2023-02-01'")).scalar()
        feb_count = conn.execute(text("SELECT count(*) FROM test_events WHERE created_at >= '2023-02-01'")).scalar()
